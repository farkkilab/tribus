...
def scoreNodes(data_to_score, labels, level, grid_size=None):
    '''scoring function'''
    level_logic_df = labels[level]
    if grid_size: # If scoring clusters use the grid size to score each node
        scores_matrix = np.zeros((grid_size*grid_size, labels[level].shape[1]))
    else: # If scoring single cells, param grid_size should be None
        scores_matrix = np.zeros((data_to_score, labels[level].shape[1]))
...
def run(input_path,labels,output_folder, level_ids, previous_labels):
    ...
    for level_id in level_ids:
        ...
        if len(sample_data) < REQUIRED_CELLS_FOR_CLUSTERING:
            ...
            data_to_score, labeled = clusterCells(sample_data, labels, level, grid_size)
            # Score clusters
